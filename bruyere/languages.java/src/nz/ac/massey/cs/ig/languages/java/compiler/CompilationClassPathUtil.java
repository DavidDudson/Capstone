...
/**
 * Utility to infer the classpath to be used by the compiler. This will find out
 * whether the code is running in a web app, or within Eclipse. Otherwise, this
 * will fail !
 * 
...
public class CompilationClassPathUtil {
	public static String getCompilationClassPath() {
		...
		if (path.contains("WEB-INF/lib")) {
			...
			// add all current classes to classpath
			String currentClasses = path.substring(0, path.length() - 3)
					+ "classes" + File.separator;
			b.append(currentClasses);
		...
		System.out
				.println("Cannot infer out compilation classpath, currently only eclipse and web apps are supported");
		return "";
	}
}
